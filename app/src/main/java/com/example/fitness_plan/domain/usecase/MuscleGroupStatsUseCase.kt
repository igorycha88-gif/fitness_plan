    ...
    suspend fun generateInsights(
        ...
        if (stats.isEmpty()) {
            return listOf(
                MuscleGroupInsight(
                    ...
                    message = "Начните тренировки, чтобы увидеть статистику по группам мышц",
                    muscleGroups = emptyList()
                )
            )
        }

        val summaries = calculateSummaries(stats, filter)
        val insights = mutableListOf<MuscleGroupInsight>()

        if (summaries.isNotEmpty()) {
            val mostTrained = summaries.maxByOrNull { it.totalVolume }
            mostTrained?.let {
                insights.add(
                    MuscleGroupInsight(
                        type = InsightType.MOST_TRAINED,
                        message = "Чаще всего вы нагружали **${it.muscleGroup.displayName}** - ${(it.percentage).toInt()}% от общего объёма",
                        muscleGroups = listOf(it.muscleGroup)
                    )
                )
            }

            val leastTrained = summaries.minByOrNull { it.totalVolume }
            leastTrained?.let {
                insights.add(
                    MuscleGroupInsight(
                        type = InsightType.LEAST_TRAINED,
                        message = "Меньше всего нагрузки на **${it.muscleGroup.displayName}**",
                        muscleGroups = listOf(it.muscleGroup)
                    )
                )
            }

            val needsAttention = summaries.filter { it.needsAttention }
            if (needsAttention.isNotEmpty()) {
                val group = needsAttention.maxByOrNull { it.daysSinceLastWorkout }!!
                insights.add(
                    MuscleGroupInsight(
                        type = InsightType.NEEDS_ATTENTION,
                        message = "**${group.muscleGroup.displayName}** не тренировались ${group.daysSinceLastWorkout} дней",
                        muscleGroups = listOf(group.muscleGroup)
                    )
                )
            }

            val totalGroups = summaries.size
            val averageVolume = summaries.map { it.totalVolume }.average()
            val balancedGroups = summaries.filter {
                Math.abs(it.totalVolume - averageVolume) < averageVolume * 0.3
            }
            if (balancedGroups.size >= totalGroups * 0.7) {
                insights.add(
                    MuscleGroupInsight(
                        type = InsightType.BALANCED,
                        message = "Отличная балансировка - нагрузка распределена равномерно!",
                        muscleGroups = emptyList()
                    )
                )
            }
        ...
        if (insights.isEmpty()) {
            insights.add(
                MuscleGroupInsight(
                    type = InsightType.GENERAL,
                    message = "Продолжайте тренироваться для получения более точной статистики",
                    muscleGroups = emptyList()
                )
            )
        }
